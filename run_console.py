...
problem = KCut_DGL(k=5, m=6, adjacent_reserve=20, hidden_dim=32, random_init_label=True, a=1)
...
def run_dqn():
    for i in tqdm(range(100)):
        ...
        log = alg.train_dqn(batch_size=64, grad_accum=20, num_episodes=1, episode_len=500, gcn_step=20, q_step=1, ddqn=False)
        if i % 3 == 0:
...
def test_model(step=100):
    test_problem = KCut_DGL(k=5, m=6, adjacent_reserve=20, hidden_dim=32, random_init_label=True, a=1)
