...
class KCut_DGL():
    ...
    def reset(self):
        self.g = generate_G(self.k, self.m, self.adjacent_reserve, self.hidden_dim, a=self.a, sample=False)['g']
        self.S = self.calc_S()
        return self.g
    ...
    def step(self, action, state=None):
        ...
        i, j = action
        i_label, j_label = [(state.ndata['label'][c] > 0).nonzero().squeeze() for c in action]
